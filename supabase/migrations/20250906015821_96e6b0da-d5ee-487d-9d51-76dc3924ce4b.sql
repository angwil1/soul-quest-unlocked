-- Security fix migration (corrected): Fix remaining database function vulnerabilities and secure public data
...
-- Secure public tables by requiring authentication (handle existing policies gracefully)
-- Remove public access to questions table and recreate with authentication
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'questions' AND table_schema = 'public') THEN
        ...
        -- Create or replace authenticated policy
...
-- Remove public access to faqs table and recreate with authentication
DROP POLICY IF EXISTS "Anyone can read FAQs" ON public.faqs;
DROP POLICY IF EXISTS "Authenticated users can read FAQs" ON public.faqs;
...
INSERT INTO public.security_events (
    ...
    jsonb_build_object(
        ...
        'functions_secured', ARRAY[
            'update_quiet_start_updated_at',
            'update_memory_vault_updated_at',
            'update_profiles_updated_at',
            'handle_mutual_swipe',
            'schedule_match_archiving',
            'schedule_risk_assessment',
            'auto_generate_match_details',
