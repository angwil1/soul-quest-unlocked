-- Fix the create_sample_matches_for_user function to resolve column ambiguity issues
...
BEGIN
    ...
    LOOP
        ...
        -- Get the match ID using table alias to avoid ambiguity
        ...
        IF NOT EXISTS (SELECT 1 FROM messages WHERE messages.match_id = match_id_var) THEN
            INSERT INTO messages (sender_id, match_id, message_text, created_at)
            VALUES 
            (sample_user_id, match_id_var, 
             CASE 
